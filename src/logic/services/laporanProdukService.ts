...
export const laporanProdukService = {
  async getProdukReport(startDate: string, endDate: string, timezone: string = 'Asia/Jakarta'): Promise<LaporanProdukData | null> {
    try {
      ...
      // 1. Data Pembelian
      const sqlPembelian = `
        SELECT pp.qty, ${datetimeFunc('p.datetime')} as created_at, p.id as p_id, s.name as supplier_name, p.status, pp.name as product_name
        FROM pembelian_produk pp
        JOIN pembelian p ON pp.purchase_id = p.id
        LEFT JOIN suplier s ON p.supplier_id = s.id
        WHERE ${dateFunc('p.datetime')} BETWEEN date(?) AND date(?)
      `;
      ...
      const sqlPenerimaan = `
        SELECT p.qty_received_actual, p.qty_rejection, p.qty_diff, ${datetimeFunc('p.datetime')} as created_at, pp.name as product_name
        FROM penerimaan p
      ...
      // 4. Stok Masuk
      const sqlStokMasuk = `
        SELECT sm.qty_in, ${datetimeFunc('sm.created_at')} as created_at, sm.sku
        FROM stok_masuk sm
        WHERE ${dateFunc('sm.created_at')} BETWEEN date(?) AND date(?)
      `;
      ...
      const sqlStokTerbuang = `
        SELECT st.name, st.qty, ${datetimeFunc('st.created_at')} as created_at
        FROM stok_terbuang st
        WHERE ${dateFunc('st.created_at')} BETWEEN date(?) AND date(?)
      `;
      ...
      // 6. Stok Retur
      const sqlStokRetur = `
        SELECT sr.name, sr.qty, ${datetimeFunc('sr.created_at')} as created_at
        FROM stok_retur sr
        WHERE ${dateFunc('sr.created_at')} BETWEEN date(?) AND date(?)
      `;
      ...
      const sqlPenjualan = `
        SELECT pp.name, pp.qty, ${datetimeFunc('p.datetime')} as created_at
        FROM penjualan_produk pp
        JOIN penjualan p ON pp.penjualan_id = p.id
        WHERE p.approval_status = 'Approved'
        AND ${dateFunc('p.datetime')} BETWEEN date(?) AND date(?) 
      `;
      ...
      const sqlPenjualanMixing = `
        SELECT ppm.name, ppm.total_qty as qty, ${datetimeFunc('p.datetime')} as created_at
      ...
      // 8. Stok Opname
      const sqlOpname = `
        SELECT so.qty_diff, ${datetimeFunc('so.created_at')} as created_at, so.sku
        FROM stok_opname so
        WHERE ${dateFunc('so.created_at')} BETWEEN date(?) AND date(?)
      `;
      ...
      const initialBalanceSql = `
        SELECT 
          ...
          (
            ...
            WHERE ${datetimeFunc('sm.created_at')} >= IFNULL(${datetimeFunc('sb2.last_so_datetime')}, '1970-01-01')
            AND ${dateFunc('sm.created_at')} < date(?)
          ) as total_in,
          (
            SELECT IFNULL(SUM(p.qty_received_actual), 0) FROM penerimaan p 
            JOIN pembelian_produk pp ON p.purchase_product_id = pp.id 
            JOIN stok_berjalan sb2 ON pp.name = sb2.name 
            WHERE ${datetimeFunc('p.datetime')} >= IFNULL(${datetimeFunc('sb2.last_so_datetime')}, '1970-01-01')
            AND ${dateFunc('p.datetime')} < date(?)
          ) as total_penerimaan,
          (
            SELECT IFNULL(SUM(sr.qty), 0) FROM stok_retur sr 
            JOIN stok_berjalan sb2 ON sr.name = sb2.name 
            WHERE ${datetimeFunc('sr.created_at')} >= IFNULL(${datetimeFunc('sb2.last_so_datetime')}, '1970-01-01')
            AND ${dateFunc('sr.created_at')} < date(?)
          ) as total_retur,
          (
            SELECT IFNULL(SUM(st.qty), 0) FROM stok_terbuang st 
            JOIN stok_berjalan sb2 ON st.name = sb2.name 
            WHERE ${datetimeFunc('st.created_at')} >= IFNULL(${datetimeFunc('sb2.last_so_datetime')}, '1970-01-01')
            AND ${dateFunc('st.created_at')} < date(?)
          ) as total_terbuang,
          (
            ...
            JOIN stok_berjalan sb2 ON pp.name = sb2.name 
            WHERE p.approval_status = 'Approved' 
            AND ${datetimeFunc('p.datetime')} >= IFNULL(${datetimeFunc('sb2.last_so_datetime')}, '1970-01-01')
            AND ${dateFunc('p.datetime')} < date(?)
          ) as total_terjual,
          (
            SELECT IFNULL(SUM(ppm.total_qty), 0) FROM penjualan_produk_mixing ppm 
            JOIN penjualan p ON ppm.penjualan_id = p.id 
          ...
          (
            ...
            WHERE ${datetimeFunc('so2.created_at')} >= IFNULL(${datetimeFunc('sb2.last_so_datetime')}, '1970-01-01')
            AND ${dateFunc('so2.created_at')} < date(?)
          ) as total_opname
        FROM stok_berjalan 
      `;
