...
self.addEventListener('push', (event) => {
  ...
  const defaultIcon = urlParams.get('icon') || '/maindi-logo.png';
  const defaultBadge = urlParams.get('badge') || defaultIcon;

  let data = { title: 'Pangan Mas Abadi', body: 'Pembaruan baru tersedia!' };
  try {
    if (event.data) {
  ...
  event.waitUntil(
    self.registration.showNotification(data.title || 'Pangan Mas Abadi', options)
  );
});
